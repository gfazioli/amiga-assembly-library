...
The complete assembly source code for the REI module (`rei.asm`).
...
* OpenREIA()       	   - New - modified (old DisplayREIA())
* EraseInternalRect()	   - New
* LayoutAsmGadget()        - PRIVATE USE
* NewFreeVisualInfo()	   - PRIVATE USE
* GetSystemHook()	   - PRIVATE USE
* CloseREI()		   - New - modified (old FreeDisplayREI())
* ActiveREI()		   - Ok
* LockREI()		   - Ok - modified
* UnlockREI()		   - Ok	- modified
* RefreshREI()		   - Needs to be redone...
* InterfaceInfo()	   - New
* LoadNewDTObjectA()	   - PRIVATE USE ONLY
* SetREIAttrsA()	   - Ok - needs checking
* GetREIAttrsA()	   - Ok
*
***************************************************************************************
...
CB_Pens	swap	d6					* Clear space for image...
	...
	beq.s	ContFit
	move.w	#STDIM_WIDTH,d6				* Standard Offset
ContFit	move.l	LN_NAME(a3),a0				* Get the Name
	STRLEN	a0,d0					* Calculate the length...
...
;--------------------------------------------------------------------------------------
	move.l	d4,a3
	move.l	d0,d4					* fit
	sub.w	te_Extent+ra_MaxY(a2),d3		* Calculating slack center...
	...
	move.w	d4,-(sp)				* Save D4 Temporarily...
	moveq	#0,d4
	move.l	d4,a0
	jsr	_LVOBltPattern(a6)
	move.w	(sp)+,d4				* Recover D4 = fit
	move.l	lvdm_RastPort(a4),a1			* RastPort
	move.w	d6,d0					* apen
	ext.l	d0
	move.l	d7,d1					* bpen
...
NoImage	move.l	lvdm_State(a4),d2			* LVR_NORMAL check...
	...
	beq.s	SafExt
;--------------------------------------------------------------------------------------
MKGhost	move.l	lvdm_RastPort(a4),a1		* This is the routine that performs
	move.l	([lvdm_DrawInfo.w,a4],dri_Pens.w),a0	* the Ghosting of an element
	...
	moveq	#0,d0
	move.l	lvdm_RastPort(a4),a1
	move.l	d0,rp_AreaPtrn(a1)		* Clear the Pattern...
...
;--------------------------------------------------------------------------------------
GhostPattern
	dc.l $44441111				* Standard Amiga Ghosting...
;--------------------------------------------------------------------------------------
...
** INTERNAL USE
**
** This is the reimplementation of the gadtools/GetVisualInfo command.
**
...
SRE_PUB	move.l	rei_PubScreenName(a4),a0
	...
	move.l	a3,d0
;--------------------------------------------------------------------------------------
; This part may change in future versions of the system, since now that we
; know the visual info structure, we only take this, calling among other things
; a private routine. So we take this VisualInfo.
;--------------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------------
; Initialize the NewWindow by inserting the info about the screen it will open on.
;--------------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------------
; The CreateContext() is always done, and before Opening the Window.
;--------------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------------
; Check if the Flag exists, to center the Window relative to the Screen.
;--------------------------------------------------------------------------------------
...
;----	FINDTAG
;----	beq.s	SFIW
;----	move.l	4(a0),d4			* Wants a new Width
...
;----	FINDTAG
;----	beq.s	SFIH
;----	move.l	4(a0),d5			* Wants a new Height
...
;--------------------------------------------------------------------------------------
; Invoke if present, the CustomHook...
;--------------------------------------------------------------------------------------
CU_HOOK	move.l	rei_CustomHook(a4),d0
	...
	jsr	(a3)
;--------------------------------------------------------------------------------------
; Asl... future implementation - Not supported at the moment.
;--------------------------------------------------------------------------------------
SRE_ASL
...
ClearREI	ds.l 8
;--------------------------------------------------------------------------------------
; These are the error exit paths... in theory they should never be
; used; later on, DisplayAlert() calls should be added to provide
; more information to the programmer.
;--------------------------------------------------------------------------------------
SRE_FREEGADGETS
...
**                A4      A3       A2
**
** This command, for exclusively internal use, takes care of performing the layout
** of the Gadgets to display in a REI (Window). The NewGadget structure present
** inside the AsmGadget structure is correctly initialized, based
** on particular info present in the AsmGadget.
**
***************************************************************************************
MAKEFONT	MACRO
	moveq	#-1,d0			* Calculate the number of characters
CLEN\@	tst.b	(a0)+
	dbeq	d0,CLEN\@
	lea	(a0,d0.l),a0		* Return to start of string...
	neg.l	d0			* D0 = number of characters
	move.l	rei_rpGadget(a4),a1	* Dummy RastPort...
...
LayoutAsmGList
	...
	asr.w	#1,d5
	add.w	d5,d6
	addq.w	#4,d6				* D6 Standard Height of Gadget...
	moveq	#0,d4
	moveq	#0,d5
	move.b	([rei_Window.w,a4],wd_BorderLeft.w),d4	* Fixed, Border
	move.b	([rei_Window.w,a4],wd_BorderTop.w),d5	* Fixed, Border
	bra.s	LAY_GO
LAY_EXT	move.l	([a3]),d0		* was this the last one I did??
	beq.s	LAY_END			* yes, exit...
	move.l	LN_SUCC(a3),a3		* ok, get the next one....
LAY_GO	move.l	agg_Kind(a3),d0		* Kind... D7 is Scratch...
...
;--------------------------------------------------------------------------------------
LAY_Table
	dc.l LAY_EXT		* GENERIC_KIND (to be tested)
...
** INTERNAL USE
**
** This is the reimplementation of the gadtools/FreeVisualInfo() command.
***************************************************************************************
NewFreeVisualInfo
	movem.l	d2/a3-a6,-(sp)
	move.l	a6,a5
...
** INTERNAL USE
**
** Gets the address of the Hook used to draw a pattern on the windows of
...
Search	move.l	wd_Flags(a0),d0
	andi.l	#WBENCHWINDOW,d0
	beq.s	NextW
	move.l	wd_Title(a0),d0
	beq.s	Found
NextW	move.l	wd_NextWindow(a0),d0
	move.l	d0,a0
	bne.s	Search
	movem.l	(sp)+,a0-a6
	rts
Found	move.l	([wd_RPort.w,a0],rp_Layer.w),a0
...
;--------------------------------------------------------------------------------------
; In this new version... we always free the memory from the Gadget list
; starting from the return of CreateContext() contained as we know in rei_glist. The
; Window at this point has already been closed.
;--------------------------------------------------------------------------------------
	move.l	rei_glist(a4),a0	* = CreateContext
	move.l	ab_GadToolsBase(a5),a6
...
;--------------------------------------------------------------------------------------
; Now let's check if a CustomScreen was opened...
;--------------------------------------------------------------------------------------
...
***************************************************************************************
* (V41.1) - 06-10-1994 =JON= **** ######NEEDS TO BE REDONE OR REMOVED########
***************************************************************************************
_LVORefreshREI
...
_LVOLoadNewDTObjectA
	...
	move.l	a0,a4			* Save the TagList...
	move.l	d0,a3			* Save the Filename...
;--------------------------------------------------------------------------------------
	move.l	ab_DataTypesBase(a5),a6
	jsr	_LVONewDTObjectA(a6)
	tst.l	d0
	bne.s	LNDExit			* Ok.. found...
;--------------------------------------------------------------------------------------
	move.l	ab_DosBase(a5),a6	* Working in Dos...
	move.l	a3,d1			* Extract only the filename, i.e. the last
	jsr	_LVOFilePart(a6)	* part of the path...
	move.l	d0,a3			* Now I have only the filename without path...
	move.l	([ab_ExecBase.w,a5],ThisTask.w),a0	* A0 = (struct Task *)
	move.l	pr_HomeDir(a0),d1	* move to the current drawer...
	jsr	_LVOCurrentDir(a6)
;--------------------------------------------------------------------------------------
	move.l	a4,a0			* Retry here...
	move.l	a3,d0
	move.l	ab_DataTypesBase(a5),a6
	jsr	_LVONewDTObjectA(a6)
	tst.l	d0
	bne.s	OkTrova			* Ok.. found...
;--------------------------------------------------------------------------------------
	...
	move.l	d0,d1
	jsr	_LVOCurrentDir(a6)	* move to sys:Classes/Images
;--------------------------------------------------------------------------------------
	move.l	a4,a0			* Retry here...
	move.l	a3,d0
	move.l	ab_DataTypesBase(a5),a6
