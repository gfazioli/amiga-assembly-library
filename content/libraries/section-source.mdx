# Image Data Source Code

The assembly source code for the static image data section (`section.asm`).

> These are the built-in UI icon images used by the REI interface. Each image is a 16x10 pixel, 3-depth (8-color) Intuition `Image` structure stored in Chip RAM, representing different file types and UI elements such as text files, pictures, sounds, devices, drawers, selection states, and input controls.
